...
pub struct ServerRegistry { path: PathBuf }

impl ServerRegistry {
    pub fn load_default() -> Result<Self> {
        let base = dirs::config_dir().context("failed to resolve config directory")?;
        Ok(Self { path: base.join("SSHDeck").join("servers.json") })
    }
    ...
    pub fn upsert(&self, server: ServerRecord) -> Result<()> {
        ...
        if let Some(existing) = servers.iter_mut().find(|item| item.id == server.id) { *existing = server; }
    ...
    pub fn touch_recent(&self, id: &str, timestamp: u64) -> Result<()> {
        let mut servers = self.list()?;
        if let Some(server) = servers.iter_mut().find(|item| item.id == id) { server.last_connected_at = Some(timestamp); }
        self.write(servers)
    }
    ...
    fn write(&self, servers: Vec<ServerRecord>) -> Result<()> {
        ...
        if self.path.exists() { fs::remove_file(&self.path).with_context(|| format!("failed to replace {}", self.path.display()))?; }
...
mod tests {
    ...
    fn upsert_delete_and_sort_servers() {
        ...
        let registry = ServerRegistry { path: dir.path().join("servers.json") };
        registry.upsert(ServerRecord { id: "a".into(), name: "Alpha".into(), host: "alpha.example".into(), user: Some("deploy".into()), port: 22, identity_file: None, group: None, favorite: false, source_alias: None, last_connected_at: Some(10) }).unwrap();
        registry.upsert(ServerRecord { id: "b".into(), name: "Beta".into(), host: "beta.example".into(), user: None, port: 2222, identity_file: None, group: Some("Prod".into()), favorite: true, source_alias: None, last_connected_at: None }).unwrap();
