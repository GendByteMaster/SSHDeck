...
export function SidebarV2({ servers, favorites, groups, query, statuses, checking, onQueryChange, onAdd, onImport, onConnect, onFavorite, onExport, onEdit, onDelete }: Props) {
  ...
  return <aside className="workbench-primary relative grid h-full shrink-0 grid-cols-[52px_minmax(0,1fr)] overflow-visible border-r border-white/[0.055] bg-[#0a0c10]/95 text-zinc-200" style={{ width: primaryWidth }}>
    ...
    </div> : view === "sftp" ? <div className="flex min-h-0 min-w-0 flex-col bg-[#0d1015]/92"><SftpBrowser servers={allServers} selectedServerId={selectedServer.id} onSelectServer={selectSftpServer} /></div> : view === "search" ? <SearchWorkspace servers={allServers} onConnectServer={(serverId) => { const server = allServers.find((item) => item.id === serverId); if (server) onConnect(server); }} /> : view === "ports" ? <PortsWorkspace servers={allServers} activeServerId={selectedServer.id} /> : view === "sessions" ? <SessionsWorkspace /> : view === "history" ? <HistoryWorkspace servers={allServers} onReconnectServer={(serverId) => { const server = allServers.find((item) => item.id === serverId); if (server) onConnect(server); }} /> : <SettingsWorkspace />}
