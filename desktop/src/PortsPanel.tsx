...
export function PortsPanel() {
  ...
  return <div className="min-h-0 overflow-y-auto px-3 py-2 [scrollbar-width:thin]" aria-label="Managed SSH ports">
    ...
    <div className="grid gap-1.5">
      {tunnels.map((tunnel) => {
        ...
        return <div key={tunnel.id} className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 rounded-lg border border-white/[0.055] bg-white/[0.018] px-3 py-2">
          ...
          <button type="button" aria-label={`Toggle auto restart for ${tunnel.name}`} title="Toggle auto restart" onClick={() => void toggleAutoRestart(tunnel).catch(() => undefined)} className={`flex h-7 items-center gap-1 rounded-lg px-2 text-[9.5px] ${tunnel.autoRestart ? "bg-[#4f7cff]/10 text-[#91a9ff]" : "text-zinc-600 hover:bg-white/[0.04] hover:text-zinc-300"}`}><RotateCw size={11} /> Auto</button>
          <button type="button" aria-label={`${running ? "Stop" : "Start"} ${tunnel.name}`} title={running ? "Stop tunnel" : "Start tunnel"} onClick={() => void toggleTunnel(tunnel.id).catch(() => undefined)} className={`grid size-7 place-items-center rounded-lg ${running ? "bg-rose-400/[0.07] text-rose-300" : "bg-emerald-400/[0.07] text-emerald-300"}`}>{running ? <Square size={11} /> : <Play size={11} />}</button>
        </div>;
      })}
