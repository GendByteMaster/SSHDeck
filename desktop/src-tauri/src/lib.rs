...
fn list_servers() -> Result<Vec<ServerRecord>, String> {
    ServerRegistry::load_default().and_then(|registry| registry.list()).map_err(|error| error.to_string())
}

#[tauri::command]
fn save_server(mut server: ServerRecord) -> Result<ServerRecord, String> {
    if server.id.trim().is_empty() { server.id = Uuid::new_v4().to_string(); }
    server.name = server.name.trim().to_owned();
    server.host = server.host.trim().to_owned();
    if server.name.is_empty() || server.host.is_empty() { return Err("server name and host are required".to_owned()); }
    if server.port == 0 { return Err("port must be greater than zero".to_owned()); }
    ServerRegistry::load_default().and_then(|registry| registry.upsert(server.clone())).map_err(|error| error.to_string())?;
    Ok(server)
}

#[tauri::command]
fn delete_server(id: String) -> Result<(), String> {
    ServerRegistry::load_default().and_then(|registry| registry.delete(&id)).map_err(|error| error.to_string())
}
...
fn import_ssh_host(alias: String) -> Result<ServerRecord, String> {
    ...
    let output = Command::new("ssh").args(["-G", &alias]).output().map_err(|error| format!("failed to run ssh -G: {error}"))?;
    ...
    let server = ServerRecord {
        id: Uuid::new_v4().to_string(), name: alias.clone(), host: host.unwrap_or(alias.clone()), user, port,
        identity_file, group: None, favorite: false, source_alias: Some(alias), last_connected_at: None,
    };
    ServerRegistry::load_default().and_then(|registry| registry.upsert(server.clone())).map_err(|error| error.to_string())?;
    Ok(server)
}
...
fn find_server(id: &str) -> Result<ServerRecord, String> {
    ServerRegistry::load_default().and_then(|registry| registry.list()).map_err(|error| error.to_string())?
        .into_iter().find(|server| server.id == id).ok_or_else(|| "server not found".to_owned())
}
...
fn terminal_start_server(server_id: String, app: AppHandle, sessions: State<'_, Sessions>) -> Result<String, String> {
    ...
    let pair = native_pty_system().openpty(PtySize { rows: 30, cols: 120, pixel_width: 0, pixel_height: 0 }).map_err(|error| error.to_string())?;
    let child = pair.slave.spawn_command(ssh_command_for(&server)).map_err(|error| error.to_string())?;
    drop(pair.slave);
    ...
    std::thread::spawn(move || {
        ...
        loop {
            match reader.read(&mut buffer) {
                Ok(0) | Err(_) => break,
                Ok(read) => { let _ = app.emit("terminal-output", TerminalOutput { session_id: event_id.clone(), data: buffer[..read].to_vec() }); }
            }
        }
    });
    sessions.0.lock().map_err(|_| "session lock poisoned".to_owned())?.insert(session_id.clone(), Session { master: pair.master, writer, child });

    let now = SystemTime::now().duration_since(UNIX_EPOCH).map(|value| value.as_secs()).unwrap_or_default();
...
fn terminal_close(session_id: String, sessions: State<'_, Sessions>) -> Result<(), String> {
    let mut sessions = sessions.0.lock().map_err(|_| "session lock poisoned".to_owned())?;
    if let Some(mut session) = sessions.remove(&session_id) { let _ = session.child.kill(); }
    Ok(())
}

#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    tauri::Builder::default().manage(Sessions::default()).invoke_handler(tauri::generate_handler![
        list_hosts, list_servers, save_server, delete_server, import_ssh_host,
        terminal_start_server, terminal_write, terminal_resize, terminal_close
    ]).run(tauri::generate_context!()).expect("error while running SSHDeck");
}
